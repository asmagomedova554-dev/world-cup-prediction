...
const required = [
  "HYBRID 2.9", "北京时间 06月22日",
  "西班牙", "沙特", "比利时", "伊朗", "乌拉圭", "佛得角", "新西兰", "埃及",
  "荷兰 5-1 瑞典", "德国 2-1 科特迪瓦", "厄瓜多尔 0-0 库拉索", "突尼斯 0-4 日本", "Brier 0.588",
  "荷兰", "瑞典", "德国", "科特迪瓦", "厄瓜多尔", "库拉索", "突尼斯", "日本",
  "美国 2-0 澳大利亚", "摩洛哥 1-0 苏格兰", "巴西 3-0 海地", "土耳其 0-1 巴拉圭",
...
if (probabilitySets.length !== 36) {
  throw new Error(`页面源码应有36组历史胜平负概率，实际为${probabilitySets.length}组`);
}

const boldScores = [...html.matchAll(/boldScore:"(\d+)\s*:\s*(\d+)"/g)];
if (boldScores.length !== 36) {
  throw new Error(`页面源码应有36个历史大胆预测比分，实际为${boldScores.length}个`);
}
