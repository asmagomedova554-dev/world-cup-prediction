...
if (probabilitySets.length !== 8) {
  throw new Error(`页面源码应有8组新旧胜平负概率，实际为${probabilitySets.length}组`);
}

const boldScores = [...html.matchAll(/boldScore:"(\d+)\s*:\s*(\d+)"/g)];
if (boldScores.length !== 8) {
  throw new Error(`页面源码应有8个新旧大胆预测比分，实际为${boldScores.length}个`);
}
